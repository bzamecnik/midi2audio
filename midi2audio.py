...
class FluidSynth():
    ...
    def midi_to_audio(self, midi_file, audio_file, gain=DEFAULT_GAIN):
        subprocess.call(['fluidsynth', '-ni', '-g', str(gain), self.sound_font, midi_file, '-F', audio_file, '-r', str(self.sample_rate)])

    def play_midi(self, midi_file, gain=DEFAULT_GAIN):
        subprocess.call(['fluidsynth', '-i', '-g', str(gain), self.sound_font, midi_file, '-r', str(self.sample_rate)])
